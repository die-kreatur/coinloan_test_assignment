...
// curl -X GET -H "Content-Type: application/json" --data-binary "{\"jsonrpc\": "\2.0\", "\method\": "\protocolVersion\", "\id\": 1 }" 127.0.0.1:3030

#[rpc]
pub trait Rpc {

     #[rpc(name = "newOrder")]
     fn add_new_order(&self, symbol: String, side: String, quantity: f64, price: f64) -> JsonRpcResult<Order>;

     #[rpc(name = "listOrders")]
     fn list_orders(&self) -> JsonRpcResult<Vec<(String, String, BigDecimal, BigDecimal, bool)>>;
...
impl Rpc for RpcImpl {
     ...
     fn add_new_order(&self, symbol: String, side: String, quantity: f64, price: f64) -> JsonRpcResult<Order> {
          ...
          Ok(create_order(&conn, &symbol, &side, &quantity, &price))
     }
